...
public class Registration extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!password.equals(repassword)){
			...
			if(hm.containsKey(username))
				...
				session.setAttribute("login_msg", "Your "+usertype+" account is create. Please login");
				if(!helper.isLoggedin()){
	...
	protected void displayRegistration(HttpServletRequest request,
		...
		pw.print("<form method='post' action='Registration'>"
				...
				+ "</td></tr><tr><td>"
				+ "<h3>User Type</h3></td><td><select name='usertype' class='input'><option value='customer' selected>Customer</option><option value='retailer'>Store Manager</option><option value='manager'>Sales Man</option></select>"
				+ "</td></tr></table>"
